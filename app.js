...
const isAdult = (x) => {
  if (x>=18){
    ...
    return "Illegal number, this person was not born yet";
...
const convertTemperature = (temperature, scale) => {
  ...
  if (scale.toLowerCase()==="c")
  {
    ...
    return `${finalTemperature} (Fahrenheit)`
  }
  else if (scale.toLowerCase()==="f")
  {
    finalTemperature=(temperature-32)*(9/5)
    return `${finalTemperature} (Celsius)`
  }
}
...
const createUsername = (firstName, lastName) => {
  let firstThreeOfFirstName=firstName.slice(0,3);
  let firstThreeOfLastName=lastName.slice(0,3);
  let totalCharacterCount=firstName.length+lastName.length;

  return `${firstThreeOfFirstName}${firstThreeOfLastName}${totalCharacterCount}`
}
...
const numArgs = (...numberOfArguments) => {
